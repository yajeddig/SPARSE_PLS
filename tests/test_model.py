...
def test_data_preprocessor_fit_transform(
    data: Tuple[ndarray, DataFrame, ndarray, List[str]],
) -> None:
    """
...
def test_data_preprocessor_pandas_structure_preserved(
    data: Tuple[ndarray, DataFrame, ndarray, List[str]],
) -> None:
    """
    ...
    assert list(X_transformed.columns) == list(
        X_df.columns
...
def test_data_preprocessor_sparse_input(sparse_data):
    ...
    try:
        ...
        assert "NaN values" not in str(
            e
        ), "Sparse input incorrectly identified as containing NaNs"


def test_data_preprocessor_custom_scaler(
    data: Tuple[ndarray, DataFrame, ndarray, List[str]],
) -> None:
    """
    ...
    X_transformed = preprocessor.fit_transform(X_np)
    assert (
        X_transformed.shape == X_np.shape
    ), "Custom scaler did not process data correctly."
...
def test_missing_value_imputation(
    ...
    assert not np.isnan(
        X_transformed
    ).any(), "Imputation did not handle missing values."
...
def test_pipeline_integration(
    ...
    pipeline = Pipeline(
        [
            (
                "preprocessor",
                DataPreprocessor(method="robust", impute_strategy="median"),
            ),
            ("model", SparsePLS(n_components=2, alpha=0.1)),
        ]
    )
    ...
    assert (
        y_pred.shape[0] == X_np.shape[0]
    ), "Pipeline prediction output shape mismatch."
...
def test_export_estimator(
    data: Tuple[ndarray, DataFrame, ndarray, List[str]], tmp_path: Path
) -> None:
    """
    ...
    assert isinstance(
        loaded_model, SparsePLS
    ), "Loaded object is not an instance of SparsePLS."


def test_get_selected_feature_names(
    data: Tuple[ndarray, DataFrame, ndarray, List[str]],
) -> None:
    """
    ...
    assert all(
        isinstance(f, (str, int)) for f in selected_features
...
def test_optimize_parameters(
    ...
    param_grid = {"n_components": [1, 2], "alpha": [0.1, 0.5]}
    ...
    assert isinstance(
        model.cv_results_, pd.DataFrame
    ), "cv_results_ should be a DataFrame."
